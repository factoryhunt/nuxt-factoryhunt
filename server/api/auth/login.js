...
module.exports = (req, res) => {
  ...
  mysql.query(`SELECT * FROM ${CONFIG_MYSQL.TABLE_CONTACTS} WHERE lower(contact_email) = "${email}"`,
    (err, users) => {
      ...
      if (!user) return res.status(401).json({result: false, msg: 'email does not exist', msg_kor: '존재하지 않는 이메일입니다.'})
      ...
      crypto.pbkdf2(password, user.password_salt, 100000, 64, 'sha512', function (err, key) {
        ...
        if (!result) return res.status(401).json({result: false, msg: 'password is incorrect', msg_kor: '비밀번호가 일치하지 않습니다.'})
