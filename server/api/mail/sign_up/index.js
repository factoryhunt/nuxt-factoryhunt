...
module.exports = async (req, res) => {
  ...
  const createToken = () => {
    return new Promise((resolve, reject) => {
      ...
      jwt.sign(payload, jwt_config.jwtSecret, options, (err, token) => {
        if (err) reject(err)
        resolve(token)
      })
    })
  ...
  const sendEmail = token => {
    return new Promise((resolve, reject) => {
      ...
      const mailOptions = {
        from: 'Factory Hunt <noreply@factoryhunt.com>', // sender address
  ...
  const onSuccess = () => {
    res.status(200).json({
      result: true,
      msg:
        'Sign up email sent success. We just sent verifying email to the email. Check the email.',
      msg_kor:
